...
export default function VentasPos() {
  ...
  const [porcentajeIva, setPorcentajeIva] = useState(0.16);
  const [vendedoresList, setVendedoresList] = useState<{id: string, nombre: string}[]>([]);
  ...
  const cargarDatosBasicos = async () => {
    ...
    try {
      ...
      // 2.5 Configuración de IVA y Vendedores
      const [{ data: configData }, { data: perfData }] = await Promise.all([
        supabase.from('configuracion').select('valor').eq('clave', 'porcentaje_iva').single(),
        supabase.from('perfiles').select('id, nombre_completo').in('rol', ['vendedor', 'admin'])
      ]);
      
      if (configData && configData.valor) {
        setPorcentajeIva(parseFloat(configData.valor) / 100);
      }
      if (perfData) {
        setVendedoresList(perfData.map(p => ({ id: p.id, nombre: p.nombre_completo })));
      }
    ...
    openPaymentModal({
      ...
      onConfirm: async (pagos, vuelto) => {
        try {
          ...
          // Fetch real correlativo to show (in background the DB function is better but we'll assign it here for now or it's generated by DB)
          // In an ideal world, the `numero_factura` is handled in a trigger, but we'll show success anyway.
        ...
        <div className="flex items-center gap-3">
          ...
          >
            {vendedoresList.map(v => (
              <option key={v.id} value={v.id} className="text-black">{v.nombre}</option>
            ))}
            {vendedoresList.length === 0 && <option value="0" className="text-black">Cargando...</option>}
          </select>
