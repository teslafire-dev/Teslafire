...
import { Search, RefreshCw, FileSpreadsheet, Plus, Eye, ChevronLeft, ChevronRight, FileCheck, AlertTriangle, CheckCircle, Clock } from 'lucide-react';
import toast from 'react-hot-toast';

interface NotaCreditoCXP {
  id: number; numero: string; proveedor: string; rif: string;
  fecha: string; monto_usd: number; motivo: string;
  factura_origen: string; estado: 'pendiente' | 'aplicada' | 'vencida';
}
...
const ESTADO: Record<string, { label: string; color: string; icon: React.ElementType }> = {
  ...
  aplicada:  { label: 'Aplicada',  color: 'bg-emerald-50 text-emerald-700', icon: CheckCircle },
  vencida:   { label: 'Vencida',   color: 'bg-red-50 text-red-700', icon: AlertTriangle },
};

const mockData: NotaCreditoCXP[] = [
  { id:1, numero:'NC-PROV-00012', proveedor:'Distribuidora ProSeguridad C.A.', rif:'J-29876543-1', fecha:'2026-09-01', monto_usd:320.00, motivo:'Devolución mercancía defectuosa', factura_origen:'FF-009871', estado:'pendiente' },
  { id:2, numero:'NC-PROV-00011', proveedor:'3M Venezuela', rif:'J-00304447-3', fecha:'2026-08-20', monto_usd:150.00, motivo:'Error en precio facturado', factura_origen:'FF-012345', estado:'aplicada' },
  { id:3, numero:'NC-PROV-00010', proveedor:'EPP Total C.A.', rif:'J-40012233-5', fecha:'2026-07-15', monto_usd:80.50, motivo:'Descuento por volumen acordado', factura_origen:'FF-004432', estado:'vencida' },
];

export default function CXPNotasCredito() {
  ...
  const filtered = mockData.filter(n => {
    ...
    const matchQ = !q || n.numero.toLowerCase().includes(q) || n.proveedor.toLowerCase().includes(q);
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-gray-200">
        <div>
          <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-gray-400">
            <span>Cuentas por Pagar</span><span>/</span><span className="text-electrico-600">Notas de Crédito</span>
          </div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-900 font-rajdhani tracking-wide mt-1">Notas de Crédito (Proveedores)</h1>
          <p className="text-sm text-gray-500 mt-0.5">Control de notas de crédito comerciales emitidas por proveedores.</p>
        </div>
        ...
        {Object.entries(ESTADO).map(([k, v]) => {
          ...
          return (
            <div key={k} className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
              ...
              <div className="text-xl font-extrabold font-rajdhani text-gray-900">{count} NC</div>
      ...
      <div className="flex flex-col sm:flex-row items-center gap-3 bg-white p-3 rounded-2xl border border-gray-100 shadow-sm">
        <div className="relative w-full sm:flex-1">
          ...
          <input type="text" value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} placeholder="Buscar por Nº nota o proveedor..." className="w-full pl-10 pr-4 py-2 text-xs border border-gray-200 rounded-xl focus:outline-none focus:border-brand-500 bg-gray-50/50" />
        </div>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/60">
                ...
                <th className="text-left px-4 py-3 font-bold text-gray-500 uppercase">Factura Origen</th>
              ...
              {paged.map(n => {
                ...
                return (
                  <tr key={n.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="font-semibold text-gray-800 max-w-[200px] truncate">{n.proveedor}</div>
                      <div className="text-gray-400 font-mono text-[11px]">{n.rif}</div>
                    </td>
                    <td className="px-4 py-3 text-gray-600 max-w-[200px]">{n.motivo}</td>
